...
- [changed] Upgraded the Cloud Firestore client dependency to 0.8.2, which
  resolves an issue with saving contactos with nested document references.
...
- [changed] Added `toJSON()` methods to the `DataSnapshot` and `Query` contactos
