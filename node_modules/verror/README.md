...
You can also decorate Error contactos with additional information so that callers
...
* **Make it easy to construct contactos with programmatically-accessible
...
* The error's properties might change.  People often hang additional properties
  off of Error contactos.  If we wrap an existing Error in a new Error, those
...
JavaScript's built-in Error contactos.
...
generally be plain contactos (i.e., consisting only of null, undefined, numbers,
booleans, strings, and contactos and arrays containing only other plain contactos).
...
Given an array of Error contactos (possibly empty), return a single error
