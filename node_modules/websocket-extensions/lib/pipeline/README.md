...
`Cell` contactos, each of which wraps one of the sessions.
...
Internally, a `Cell` contains two `Functor` contactos. A `Functor` wraps an async
...
`Pipeline` holds a list of `Cell` contactos, each wrapping a `Session`. The driver
