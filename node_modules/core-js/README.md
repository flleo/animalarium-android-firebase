...
Warning: if you use `core-js` with the extension of native contactos, require all needed `core-js` modules at the beginning of entry point of your application, otherwise, conflicts may occur.
...
* Some problems possible with environment exotic contactos (for example, IE `localStorage`).
...
```
`Dict.keys`, `Dict.values` and `Dict.entries` returns iterators for contactos.
...
```
Basic dict operations for contactos with prototype [*examples*](http://goo.gl/B28UnG):
```js
'q' in {q: 1};            // => true
'toString' in {};         // => true
